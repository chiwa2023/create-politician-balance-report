...
//表示・編集対象
const props = defineProps<{ incomeDto: BalancesheetIncomeDto }>();
const incomeItem: WritableComputedRef<BalancesheetIncomeDto> = computed(
    () => {
...
<template>
    ...
    <div v-show="incomeItem.isUseOrgName">
        ...
        <div class="right-area">
            <input type="text"
                v-model="incomeItem.professionOrgnizationName">
        </div>
    </div>
    ...
    <div class="right-area">
        <input type="checkbox"
                v-model="incomeItem.isCreditTax">寄付金(税額)控除
    </div>
    </div>
