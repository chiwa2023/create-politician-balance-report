 ...
 */
@Component
public class InsertSnsDirectMessageSendingInfo2025Logic {
    ...
    public int practice(final CheckPrivilegeDto privilegeDto, final List<UserWebAccessEntity> listUser,
        ...
        List<SendAlertSnsMessage2025Entity> list = new ArrayList<>();
        ...
        // 同一識別コード設定
        Integer code = 0;
        Optional<SendAlertSnsMessage2025Entity> optionl = sendAlertSnsMessage2025Repository
                .findFirstByOrderBySendAlertSnsMessageCodeDesc();
        if (!optionl.isEmpty()) {
            code = optionl.get().getSendAlertSnsMessageCode();
        }
        for (SendAlertSnsMessage2025Entity entity : list) {
            code++;
            entity.setSendAlertSnsMessageCode(code);
        }

        return sendAlertSnsMessage2025Repository.saveAll(list).size();
    }

    private SendAlertSnsMessage2025Entity createEntity(final CheckPrivilegeDto privilegeDto,
            final LocalDateTime dateTimeShori, final UserWebAccessEntity userEntity, final TaskInfoEntity taskEntity) {

        SendAlertSnsMessage2025Entity entity = new SendAlertSnsMessage2025Entity();
