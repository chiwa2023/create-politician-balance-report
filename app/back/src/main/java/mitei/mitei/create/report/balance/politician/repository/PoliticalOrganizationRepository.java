...
public interface PoliticalOrganizationRepository extends JpaRepository<PoliticalOrganizationEntity, Long> {
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT * FROM political_organization WHERE saishin_kbn= 1 AND MATCH(table_all_search_text) AGAINST (?1 IN NATURAL LANGUAGE MODE)", nativeQuery = true)
