package mitei.mitei.create.report.balance.politician.task_plan.logic.y2025;
...
/**
 * SearchTaskPlanByPoliticalOrgY2025Logic単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class SearchTaskPlanByPoliticalOrgY2025LogicTest {
    // CHECKSTYLE:OFF
    ...
    @Test
    @Transactional
    @Sql("task_plan_2025.sql")
    void testPractice() {

        Integer userCode = 890;

        List<Integer> listPoliOrg = new ArrayList<>();
        List<TaskPlanSelectOptionDto> listDto00 = searchTaskPlanByPoliticalOrgY2025Logic.practice(listPoliOrg,
                userCode);
        assertTrue(listDto00.isEmpty(), "政治団体に所属していないデータを検索した場合は空リストが返る");

        listPoliOrg.add(3570);
        List<TaskPlanSelectOptionDto> listDto01 = searchTaskPlanByPoliticalOrgY2025Logic.practice(listPoliOrg,
                userCode);
        ...
        List<TaskPlanSelectOptionDto> listDto02 = searchTaskPlanByPoliticalOrgY2025Logic.practice(listPoliOrg01,
                userCode);
