...
/**
 * InsertMailInfoFactoryLogic単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class InsertMailInfoFactoryLogicTest {
    // CHECKSTYLE:OFF
    ...
    @Test
    @Transactional
    @Sql("y2024/truncate_alert_mail_2024.sql")
    void testPractice() { // NOPMD
        
        //TODO 違う報告年テーブルへのアクセスするテストは自動で追加する
        ...
        int size = insertMailInfoFactoryLogic.practice(privilegeDto, shori, user00, listUser, listTask);
        assertThat(size).isEqualTo(4); // 2×2の4件

        List<SendAlertMail2024Entity> list = sendAlertMail2024Repository.findAll();
        list.sort((e1, e2) -> e1.getSendAlertMailCode() - e1.getSendAlertMailCode());

        // 1件目
        SendAlertMail2024Entity result00 = list.get(0);
        ...
        // 1件目
        SendAlertMail2024Entity result01 = list.get(1);
        ...
        // 3件目
        SendAlertMail2024Entity result02 = list.get(2);
        ...
        // 4件目
        SendAlertMail2024Entity result03 = list.get(3);
