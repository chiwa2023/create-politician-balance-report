...
/**
 * SearchTaskPlanByPoliticalOrgY2022Logic単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class SearchTaskPlanByPoliticalOrgY2022LogicTest {
    // CHECKSTYLE:OFF
    ...
    @Test
    @Transactional
    @Sql("task_plan_2022.sql")
    void testPractice() {

        Integer userCode = 890;

        List<Integer> listPoliOrg = new ArrayList<>();
        List<TaskPlanSelectOptionDto> listDto00 = searchTaskPlanByPoliticalOrgY2022Logic.practice(listPoliOrg,
                userCode);
        assertTrue(listDto00.isEmpty(), "政治団体に所属していないデータを検索した場合は空リストが返る");

        listPoliOrg.add(3570);
        List<TaskPlanSelectOptionDto> listDto01 = searchTaskPlanByPoliticalOrgY2022Logic.practice(listPoliOrg,
                userCode);
        ...
        List<TaskPlanSelectOptionDto> listDto02 = searchTaskPlanByPoliticalOrgY2022Logic.practice(listPoliOrg01,
                userCode);
