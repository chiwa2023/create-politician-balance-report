...
(1)既存政治団体が乗り換えてくる場合と、(2)新規政治団体設立の場合、(a)すでに調査側で団体登録が行われている場合と(b)団体登録が存在しない場合が存在するので、それぞれのシナリオに対応する必要がある

1. 会計責任者または代表者がユーザ登録を行う
2. 政治団体の登録に必要な情報を揃える。
a. XMLを使用して団体情報を読みとる
b. データがない場合は簡易情報の手入力をしてもらう
3. 調査側ですでに団体登録が行われているか確認する
4. 団体登録が行われているのであれば、ユーザの政治団体の紐づけを行う
5. 団体登録が行われていない場合は、追加で詳細情報も入力してもらい団体登録を行う(XML読み取りの場合は一切追加手入力の必要をなくす)
6. 団体登録を行うと同時にユーザと政治団体の紐づけが完了

```plantuml
@startuml
start

:会計責任者または代表者がユーザ登録を行う;
:団体の責任者になるために必要な責任者登録を行う;
if(政治団体が既存の団体である) then(既存の団体である)
  if(政治資金報告書のXMLデータを保持している) then (保持している)
    :XMMLデータを読みとらせる;
    if(調査側ですでに団体登録がある) then (団体登録がある)
      :すでにある調査側のデータに紐づけ(ある種のマージ)を行う;
    else (団体登録がない)
      :詳細情報を入力してもらい団体登録を行う;
    endif
  else (保持していない)
...
:政治団体設立届に必要な情報を入力する;
:保存して提出;
  if(オンライン提出に対応している) then(対応している)
  :オンラインでデータを提出;
  else(オンライン提出不可)  
  :印刷して窓口に提出する;
  endif
  :団体設立(利用ユーザと政治団体の紐づけ完了);
endif
:登録完了;
end
@enduml
